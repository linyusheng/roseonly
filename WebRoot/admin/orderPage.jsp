...
<form id="selectform" action="orderManager" method="post">
	<div style="float：left;margin-right: 20px;">
		<ul class="pagination" style="margin: 0;">
			<c:choose>
				...
				<c:otherwise>
					<li><a href="orderManager?method=pageFindOrder&page=${pageVO.page-1}">&laquo;</a></li>	
				</c:otherwise>
			</c:choose>
			<c:forEach begin="1" end="${pageVO.totalPage}" varStatus="i">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="orderManager?method=pageFindOrder&page=${i.index}">${i.index }</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:choose>
				<c:when test="${pageVO.page < pageVO.totalPage}">
					<li><a href="orderManager?method=pageFindOrder&page=${pageVO.page+1}">&raquo;</a></li>
				</c:when>
				<c:otherwise>
